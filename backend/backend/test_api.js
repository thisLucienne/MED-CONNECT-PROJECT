...
const testPatientEmail = `patient.test.${timestamp}@test.com`;
const testDoctorEmail = `doctor.test.${timestamp}@test.com`;
...
async function testPatientRegistration() {
  try {
    ...
    console.error('❌ Erreur inscription patient:', error.response?.data || error.message);
    return false;
  }
}
...
async function testDoctorRegistration() {
  try {
    ...
    const doctorData = {
      ...
      licenseNumber: `MED-2024/${timestamp}`,
      phone: '6 98 76 54 32'
    };
    ...
    console.error('❌ Erreur inscription médecin:', error.response?.data || error.message);
    return false;
  }
}
...
async function testPatientLogin() {
  try {
    ...
    console.error('❌ Erreur login patient:', error.response?.data || error.message);
    return false;
  }
}
