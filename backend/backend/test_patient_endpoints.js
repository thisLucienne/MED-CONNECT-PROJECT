...
async function testPatientEndpoints() {
    ...
    try {
        ...
        if (!loginResponse.data.success) {
            console.log('❌ Échec de la connexion:', loginResponse.data.error?.message);
            return;
        }

        // Vérifier si 2FA est requis
        if (loginResponse.data.data.user.requiresVerification) {
            console.log('🔐 2FA requis - récupération du code depuis les logs...');
            
            // Pour les tests, on va utiliser un code fixe ou récupérer depuis la DB
            // En production, l'utilisateur entrerait le code reçu par email
            const userId = loginResponse.data.data.user.id;
            
            // Simuler la vérification 2FA avec un code test
            console.log('⚠️  2FA requis mais pas implémenté dans ce test');
            console.log('   Utilisez le code affiché dans les logs du serveur');
            return;
        }
