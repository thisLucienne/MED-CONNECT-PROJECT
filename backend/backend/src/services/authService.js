...
class AuthService {
  ...
  static async registerPatient(userData, profilePicture = null) {
    try {
      ...
      // Si la 2FA est activée, ne pas retourner les tokens directement
      if (user.isActive2FA) {
        // Générer et envoyer le code 2FA
        const twoFACode = CryptoUtils.generate2FACode();
        ...
        await EmailService.send2FACode(user, twoFACode);
      ...
      // Si pas de 2FA, retourner les tokens directement
      // Générer les tokens
