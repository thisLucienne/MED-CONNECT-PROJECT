...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, onCreateAccount, onTwoFARequired }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        Alert.alert('Erreur de connexion', response.error?.message || 'Une erreur est survenue');
      }
    } catch (error: any) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        Alert.alert('Erreur', response.error?.message || 'Impossible d\'envoyer l\'email');
      }
    } catch (error: any) {
          ...
          >
            {isLoading ? (
              <View style={styles.loadingContainer}>
                ...
                <Text style={[styles.loginButtonText, { marginLeft: 8 }]}>Connexion...</Text>
              </View>
            ) : (
