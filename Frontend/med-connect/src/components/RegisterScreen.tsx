...
interface RegisterScreenProps {
  onRegister: () => void;
  onBackToLogin: () => void;
  onTwoFARequired: (userId: string, email: string) => void;
}

const RegisterScreen: React.FC<RegisterScreenProps> = ({ onRegister, onBackToLogin, onTwoFARequired }) => {
    ...
    try {
      ...
      if (response.success) {
        if (response.data.user.requiresVerification) {
          // 2FA requis après inscription
          Alert.alert(
            'Inscription réussie !',
            'Votre compte a été créé avec succès. Un code de vérification a été envoyé à votre adresse email.',
            [
              {
                ...
                onPress: () => {
                  ...
                  onTwoFARequired(response.data.user.id, response.data.user.email);
                }
              }
            ]
          );
        } else {
          // Inscription réussie sans 2FA
          Alert.alert(
            'Inscription réussie !',
          ...
          >
            {isLoading ? (
              <View style={styles.loadingContainer}>
                ...
                <Text style={[styles.registerButtonText, { marginLeft: 8 }]}>Création en cours...</Text>
              </View>
            ) : (
